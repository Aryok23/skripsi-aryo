...
manual melalui kamera CCTV yang diamati operator. Pendekatan ini memiliki 
...
\cite{khan2020crowd}. Keterbatasan ini terasa makin berat pada periode puncak kunjungan, 
ketika lonjakan pengunjung menuntut pengawasan terus-menerus oleh petugas dalam waktu lama, 
sehingga beban kerja meningkat tajam justru pada saat informasi kepadatan paling dibutuhkan. 
Akibatnya, informasi kepadatan yang objektif dan tersedia secara 
langsung untuk banyak titik sekaligus sulit diperoleh, dan keputusan pengendalian kerumunan 
cenderung bersifat reaktif terhadap kondisi yang sudah terjadi di lapangan, bukan didasarkan 
pada data kuantitatif yang tersedia secara langsung.
...
\textit{City Brain} sejak tahun 2016, yang memproses data \textit{real-time} dari kamera dan 
...
berbasis kamera CCTV telah terbukti mampu memantau kerumunan secara otomatis. 
...
truk, dibangun dari rekaman kamera CCTV kawasan Malioboro. Beberapa varian model 
...
memproses \textit{stream} video kamera CCTV secara \textit{real-time} dan menampilkan jumlah objek 
...
otomatis di kawasan Malioboro yang masih dikelola secara manual, terdapat tiga persoalan 
pokok yang saling berurutan, mulai dari ketersediaan data, pemilihan model deteksi, hingga 
penerapannya pada sistem. Tiap persoalan diuraikan beserta dampak yang ditimbulkannya dan 
alasan mengapa persoalan tersebut perlu segera diselesaikan. Ketiga persoalan tersebut 
dirumuskan sebagai berikut.

\begin{enumerate}
    \item Dataset deteksi objek untuk sembilan kelas khas kawasan Malioboro, yaitu andong, bajaj, becak, sepeda, bus, mobil, motor, orang, dan truk, belum tersedia karena sebagian kelas tersebut tidak terdapat pada dataset publik berskala besar seperti COCO. Tanpa dataset lokal, model siap pakai cenderung gagal mengenali kelas khas seperti andong dan becak, sehingga objek tersebut terlewat atau salah klasifikasi dan estimasi kepadatan yang menjadi dasar pengambilan keputusan menjadi tidak akurat sejak akarnya. Persoalan ini perlu diselesaikan lebih dahulu karena dataset merupakan fondasi bagi seluruh tahap berikutnya, sehingga kekeliruan pada tahap ini akan merambat ke pemilihan model maupun keluaran sistem.
    \item Varian model YOLOv8 dan YOLO11 yang memberikan performa deteksi terbaik untuk kesembilan kelas tersebut belum diketahui, termasuk pengaruh ketidakseimbangan jumlah \textit{instance} antarkelas terhadap performa deteksi pada tiap kelas. Pemilihan model tanpa perbandingan yang sistematis berisiko menghasilkan model yang terlalu berat sehingga gagal berjalan secara \textit{real-time} pada perangkat dengan sumber daya terbatas, atau model yang lemah pada kelas minoritas sehingga sistem gagal mendeteksi kepadatan justru pada kondisi yang ramai. Persoalan ini mendesak karena kesalahan pemilihan model pada tahap awal sulit dikoreksi setelah sistem dibangun di atasnya.
    \item Model deteksi terbaik belum diimplementasikan ke dalam sistem pemantauan kepadatan berbasis web yang memproses \textit{stream} video kamera CCTV secara \textit{real-time}. Tanpa implementasi tersebut, model dengan akurasi setinggi apa pun tidak memberi manfaat operasional, dan pemantauan kepadatan tetap bertumpu pada pengamatan manual dengan keterbatasan cakupan serta beban pengawasan operator yang telah diuraikan pada latar belakang. Persoalan ini perlu segera dijawab karena lonjakan pengunjung pada periode puncak menuntut informasi kepadatan yang cepat dan objektif sebagai dasar pengendalian kerumunan, yang tidak dapat diandalkan dari pengamatan manual secara terus-menerus.
\end{enumerate}
...
hingga penerapan model terpilih ke dalam sistem yang berfungsi. Susunan berjenjang ini 
memastikan keluaran tiap tahap menjadi masukan bagi tahap berikutnya, sehingga capaian 
penelitian dapat ditelusuri secara runtut dari ketersediaan data hingga sistem yang 
beroperasi.

\begin{enumerate}
    \item Membangun dataset deteksi sembilan kelas objek kawasan Malioboro (andong, bajaj, becak, sepeda, bus, mobil, motor, orang, dan truk) dari rekaman kamera CCTV melalui anotasi semiotomatis berbantuan model \textit{pretrained} dengan koreksi manual.
    \item Membandingkan performa varian model YOLOv8 dan YOLO11 dalam mendeteksi kesembilan kelas menggunakan metrik mAP50, mAP50-95, dan F1, menganalisis pengaruh ketidakseimbangan kelas terhadap performa deteksi tiap kelas, lalu memilih model terbaik dan mengoptimasi performanya melalui penyetelan resolusi citra masukan.
    \item Mengimplementasikan model deteksi terbaik ke dalam sistem pemantauan kepadatan berbasis web yang memproses \textit{stream} video kamera CCTV secara \textit{real-time} dan mengukur performa inferensinya.
\end{enumerate}
...
\begin{enumerate}
    \item Lokasi penelitian dibatasi pada kawasan wisata Malioboro, Kota Yogyakarta, dengan sumber data berupa \textit{stream} video dari 22 titik kamera \textit{closed-circuit television} (CCTV) publik di kawasan tersebut.
    ...
    \item Dataset yang digunakan adalah data primer yang dibangun dari rekaman kamera CCTV kawasan Malioboro melalui anotasi semiotomatis dengan koreksi manual. Rincian proses anotasi dijelaskan pada Bab III.
    \item Sistem hanya melakukan pemantauan (\textit{monitoring}) kepadatan secara \textit{real-time} dan tidak melakukan prediksi kepadatan pada waktu mendatang.
...
\begin{enumerate}
    \item Sistem pemantauan yang dibangun dapat membantu pengelola kawasan Malioboro memperoleh informasi kepadatan pengunjung secara \textit{real-time} sebagai dasar pengambilan keputusan pengendalian kerumunan.
...
YOLO11, metrik evaluasi deteksi, ketidakseimbangan kelas, anotasi semiotomatis, dan 
...
data berupa \textit{stream} kamera CCTV, dilanjutkan anotasi semiotomatis dengan koreksi 
...
varian model, serta penyetelan resolusi citra masukan pada model terpilih untuk 
mengoptimasi performanya. Bab ini juga memuat perancangan dan implementasi sistem 
pemantauan berbasis web yang memproses video secara \textit{real-time}.
...
selanjutnya menyajikan hasil penyetelan resolusi citra masukan
