...
waktu ini dipilih agar data mencakup variasi kondisi antar hari, antar jam, dan antar cuaca, 
...
Ketiga aspek tersebut ditetapkan seragam untuk seluruh varian agar perbandingan antar 
arsitektur berlangsung pada kondisi yang terkendali.
...
augmentasi. Penyeragaman ini menjaga agar satu-satunya faktor pembeda antar varian adalah 
...
strategi pengendaliannya. Penyetelan ini diposisikan sebagai langkah optimasi terhadap satu 
model terpilih, bukan pelatihan ulang seluruh varian, sehingga lingkupnya tetap terkendali.
...
kepada pengguna. Keterhubungan antar komponen ini ditunjukkan pada Gambar 
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \node (be) [blok, right=2.2cm of cctv] {\textit{Backend} FastAPI\\(pengambilan \textit{stream}, inferensi YOLO)};
        \node (fe) [blok, right=2.2cm of be] {\textit{Frontend} React\\(video, grafik, notifikasi)};
...
lalu menutup \textit{stream} tersebut dan melanjutkan ke kamera berikutnya. Interval antar giliran 
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[1]
        ...
        \State $\Delta \gets W / |K|$ \Comment{interval antar giliran}
...
pada \textit{Google Earth} sebagai dasar transformasi homografi. Selain itu, kedua titik 
...
dipetakan ke tingkat pelayanan pada Tabel \ref{tab:los}.
