...
Seluruh nilai pada tabel ini dihitung pada himpunan uji agar perbandingan antarvarian 
...
perbandingan relatif antarvarian, bukan kecepatan pada perangkat penerapan yang dibahas 
...
\section{Hasil Penyetelan Resolusi Masukan Model Terpilih}
...
satu perubahan konfigurasi tunggal. Kenaikan terbesar terjadi pada 
...
keperluan pengujian logika notifikasi pada tahap ini, bukan nilai yang dipakai pada 
operasional sebenarnya. Ambang sesungguhnya dihitung dari luas area pejalan kaki yang telah 
diukur pada subbab \ref{sec:sistem}, yaitu 169~m\textsuperscript{2} untuk kamera seberang 
Gedung Agung dan 149~m\textsuperscript{2} untuk kamera Titik Nol, dibagi luas minimum 1,4 
m\textsuperscript{2} per orang pada Standar D. Pembagian ini menghasilkan ambang sekitar 
120 orang untuk kamera seberang Gedung Agung dan 106 orang untuk kamera Titik Nol. Pada 
tahap pengujian antarmuka ini, ambang 5 dipilih sebagai pengganti sementara agar logika 
notifikasi dapat dipicu dan diverifikasi secara cepat tanpa bergantung pada kondisi 
keramaian tertentu, sedangkan ambang berbasis Standar D yang sesungguhnya, yaitu 120 dan 
106 orang, tetap menjadi nilai yang diterapkan pada konfigurasi akhir sistem. Pengujian 
notifikasi pada ambang sesungguhnya tersebut dibahas tersendiri pada subbab 
\ref{sec:hasil_beban_tinggi}.
...
ditinjau ulang oleh operator. Dengan demikian, ketiga pengujian fungsional pada subbab ini, 
yaitu antarmuka deteksi langsung, grafik kepadatan, dan riwayat peringatan, menunjukkan 
seluruh komponen sistem berfungsi sesuai rancangan pada kondisi kepadatan biasa. Pengujian 
berikutnya menelaah perilaku sistem pada kondisi kepadatan tinggi yang menjadi sasaran utama 
sistem.

\subsection{Pengujian Notifikasi pada Beban Kepadatan Tinggi}
\label{sec:hasil_beban_tinggi}

Pengujian fungsional pada subbab \ref{sec:hasil_sistem} memverifikasi logika notifikasi 
menggunakan ambang yang diturunkan, sedangkan pengujian pada subbab ini bertujuan menelaah 
perilaku sistem ketika kepadatan sungguhan mencapai ambang berbasis Standar D yang 
sesungguhnya, yaitu kondisi beban tinggi yang menjadi sasaran utama sistem. Pengujian 
dilakukan dengan menjalankan sistem secara langsung pada \textit{stream} kamera Titik Nol 
pada malam hari, saat kepadatan pejalan kaki di kawasan tersebut meningkat secara alami 
hingga melampaui ambang 106 orang yang dihitung pada subbab \ref{sec:hasil_sistem}. Kondisi 
ini memungkinkan pengujian dilakukan pada ambang operasional yang sebenarnya, bukan ambang 
pengganti yang diturunkan.

Gambar \ref{fig:beban_dasbor} menunjukkan tampilan \textit{dashboard} saat kepadatan 
mencapai kondisi tersebut. Panel ringkasan mencatat 109 orang dan satu motor pada 
\textit{frame} saat itu, dan panel peringatan kamera menampilkan notifikasi berstatus padat 
dengan keterangan jumlah orang melebihi ambang 106. Notifikasi terpicu secara otomatis tanpa 
penurunan ambang, yang menunjukkan mekanisme deteksi kepadatan bekerja sebagaimana dirancang 
pada kondisi beban yang menjadi target operasional sistem.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/dasbor_beban_tinggi_titik_nol.png}
    \caption{Tampilan \textit{dashboard} saat kepadatan kamera Titik Nol mencapai 109 orang, dengan notifikasi padat terpicu pada ambang sesungguhnya 106 orang.}
    \label{fig:beban_dasbor}
\end{figure}

Detail peringatan yang tersimpan ditunjukkan pada Gambar \ref{fig:beban_detail}. Peringatan 
tercatat berasal dari kamera Titik Nol pada pukul 20.46 WIB dengan jenis padat, nilai pemicu 
108 orang, dan deskripsi jumlah orang melebihi ambang 106. Citra yang tersimpan 
memperlihatkan kepadatan pejalan kaki yang tinggi dengan kotak deteksi tergambar pada 
sebagian besar area \textit{frame}. Selisih kecil antara jumlah pada panel \textit{dashboard} 
saat itu dan nilai pemicu yang tersimpan, yaitu 109 berbanding 108, terjadi karena keduanya 
diambil dari \textit{frame} yang berbeda dalam selang waktu yang sangat berdekatan, sehingga 
wajar pada \textit{stream} langsung yang jumlah deteksinya berubah tiap \textit{frame}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/detail_peringatan_beban_tinggi_titik_nol.png}
    \caption{Detail peringatan beban tinggi kamera Titik Nol beserta citra CCTV dan kotak deteksinya.}
    \label{fig:beban_detail}
\end{figure}

Peringatan ini juga tercatat pada halaman riwayat, sebagaimana ditunjukkan pada Gambar 
\ref{fig:beban_riwayat}, sehingga kejadian beban tinggi tetap dapat ditelusuri ulang oleh 
operator setelah kondisi berlalu. Hal ini menunjukkan jalur penyimpanan riwayat tetap 
berfungsi pada kondisi kepadatan tinggi, sama seperti pada kondisi kepadatan rendah yang 
diuji pada subbab \ref{sec:hasil_grafik_riwayat}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{contents/chapter-4/riwayat_peringatan_beban_tinggi.png}
    \caption{Halaman riwayat peringatan yang memuat kejadian beban tinggi kamera Titik Nol pada ambang sesungguhnya.}
    \label{fig:beban_riwayat}
\end{figure}

Pengujian ini menegaskan dua hal. Pertama, dari sisi rekayasa perangkat lunak, \textit{backend} 
dan jalur notifikasi tetap berjalan stabil saat memproses \textit{frame} dengan lebih dari 
seratus objek terdeteksi, tanpa kegagalan pengiriman notifikasi maupun gangguan pada 
penyimpanan riwayat. Hal ini sejalan dengan karakteristik deteksi objek satu tahap seperti 
YOLO, yang melakukan satu kali inferensi per \textit{frame} terlepas dari jumlah objek yang 
akhirnya terdeteksi, sehingga beban komputasi inti tidak meningkat seiring bertambahnya 
jumlah objek pada \textit{frame}. Kedua, dari sisi fungsi sistem, notifikasi kepadatan 
terbukti dapat terpicu pada ambang operasional yang sesungguhnya dan bukan hanya pada ambang 
yang diturunkan, sehingga validitas mekanisme notifikasi pada beban tinggi tidak lagi 
bertumpu pada pengujian dengan ambang pengganti semata.

Pengujian beban tinggi ini memiliki keterbatasan yang perlu diakui secara jujur. Pada 
kepadatan setinggi itu, akurasi jumlah deteksi tidak dapat diverifikasi satu per satu secara 
manual, karena kotak deteksi dan label kelas saling menutupi sebagian besar area \textit{frame} 
sehingga sulit memastikan tidak ada objek yang terhitung ganda maupun terlewat. Jumlah yang 
ditampilkan tampak wajar secara visual untuk kepadatan sepadat itu, tetapi penelitian ini 
tidak mengklaim ketepatan hitungan yang presisi pada kondisi ekstrem tersebut. Selain itu, 
pengujian baru tercapai pada satu kamera, yaitu Titik Nol, karena kepadatan yang melampaui 
ambang sesungguhnya bergantung pada kondisi keramaian nyata di lapangan yang tidak dapat 
diatur waktunya. Verifikasi akurasi penghitungan pada kepadatan tinggi diajukan sebagai arah 
pengembangan lanjutan pada Bab V. Dengan rangkaian pengujian fungsional dan pengujian beban 
tinggi ini, sistem terbukti berjalan sesuai rancangan baik pada kondisi kepadatan biasa 
maupun pada ambang operasional yang sesungguhnya, sehingga tujuan ketiga penelitian berhasil 
diwujudkan.
...
implementasi sistem. Subbab ini merangkum keterkaitan antarhasil tersebut, untuk menegaskan 
...
dari kombinasi pemilihan arsitektur yang tepat dan optimasi resolusi masukan yang 
...
berhasil direkonstruksi dari data tersimpan. Lebih jauh, pengujian pada beban kepadatan 
tinggi di kamera Titik Nol pada subbab \ref{sec:hasil_beban_tinggi} menunjukkan notifikasi 
tetap terpicu dengan benar dan \textit{backend} tetap stabil saat memproses lebih dari 
seratus objek pada satu \textit{frame}, sehingga validitas sistem tidak lagi bertumpu pada 
pengujian dengan ambang yang diturunkan semata. Secara keseluruhan, ketiga tujuan penelitian 
